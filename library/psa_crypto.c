...
*/
static psa_status_t psa_generate_derived_ecc_key(
    psa_key_slot_t *slot,
    size_t bits,
    psa_key_derivation_operation_t *operation,
    uint8_t **data,
    unsigned *result)
{
    ...
    if( ( status = mbedtls_ecp_group_load( &ecp.grp, grp_id ) ) != 0 )
    {
        ret = status;
        goto cleanup;
    }
    ...
    //size_t k_len = mbedtls_mpi_size( &k );
    MBEDTLS_MPI_CHK(mbedtls_mpi_lt_mpi_ct( &k, &diff_N_2, result ) ); // this probably needs size adaptation
    ...
    psa_key_derivation_operation_t *operation )
{
    ...
    {
gen_ecc_key:
        status = psa_generate_derived_ecc_key(slot, bits, operation, &data, &key_ok);
        if( status != PSA_SUCCESS )
            goto exit;
        if (!key_ok)
            goto gen_ecc_key;
